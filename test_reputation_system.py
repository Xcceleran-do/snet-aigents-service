...
sys.path.append("./reputation")
from reputation import settings

#sys.path.append("reputation/config")

#import reputation.config.settings
...
def get_ranks(channel):
    stub = pb2_grpc.AigentsNewsFeedStub(channel)
    r = pb2.RankRequest(date=str(date(2011, 6, 16)), ids = "901")
...
def update_ranks(channel):
    stub = pb2_grpc.AigentsNewsFeedStub(channel)
    r = pb2.RankUpdate(date=str(date(2002, 6, 11)))
...
def put_ratings(channel):
    ...
    r = pb2.Ratings(
        rating_val=[
            pb2.Rating(from_rate='425', type_rate='rating', to_rate='366', value=6, weight=4, time=str(date(2010, 11, 6))),
            pb2.Rating(from_rate='122', type_rate='rating', to_rate='652', value=8, weight=5, time=str(date(2012, 12, 11)))
        ]
    )
